...
e1000_transmit(struct mbuf *m)
{
  ...
  struct tx_desc* descriptor = &tx_ring[tail];
  if (descriptor->status & E1000_TXD_STAT_DD) {
    // the descriptor has been finished
    // free the processed mbuf
    if (tx_mbufs[tail])
    {
...
e1000_recv(void)
{
  ...
  while (1) {
    ...
    descriptor->addr = (uint64) rx_mbufs[tail]->head;
    descriptor->status = 0;
